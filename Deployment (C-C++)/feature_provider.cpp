...
TfLiteStatus FeatureProvider::PopulateFeatureData(int32_t last_time_in_ms,
  ...
  // Get the number of slice that we need to fill (number of last slice filled)
  // figure out which audio data we need to fetch.
  const int last_step = (last_time_in_ms / kFeatureSliceStrideMs);
  // Number of new 16ms locations from which we can take 32ms whole samples
  int slices_needed =
  ...
  if (is_first_run_) {
    TfLiteStatus init_status = kTfLiteOk; 
  ...
  // if we need more slices than one infrence worth (*63*, 80)
  ...
  *how_many_new_slices = slices_needed;
  ...
  // last time = 64ms          current time = 96 -> we left two empty slices to get the new ones
  // +-----------+             +-----------+
  // | data@16ms |         --> | data@48ms |
  // +-----------+       --    +-----------+
  // | data@32ms |     --  --> | data@64ms |
  ...
  // if we want to keep slices -> move them to the back of featureData to fit new slices
  if (slices_to_keep > 0) { 
