...
RecognizeCommands::RecognizeCommands(int32_t average_window_duration_ms,
    ...
    : average_window_duration_ms_(average_window_duration_ms),
      ...
      previous_results_() {
  previous_top_label_ = kCategoryLabels[0];  // silence - lol
...
TfLiteStatus RecognizeCommands::ProcessLatestResults(
  ...
  if (previous_top_label_time_ == std::numeric_limits<int32_t>::min()) {
    time_since_last_top = std::numeric_limits<int32_t>::max();
  } else {
    time_since_last_top = current_time_ms - previous_top_label_time_;
  }
  if ((current_top_score > detection_threshold_) && // you need to be sure
      ((current_top_label != previous_top_label_) ||(time_since_last_top > suppression_ms_))) { // and don't repeat pred. unless enough time passed.
